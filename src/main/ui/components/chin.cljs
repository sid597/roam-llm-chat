  ...
  (let [dismiss-popover? (if (some? callback) true false)]
   [:div.chin
    ...
    [:> ButtonGroup
     ...
     #_(when (some? extract-query-pages?)
         [:> Divider]
         [:div.chk
          {:style {:align-self "center"
                   :margin-left "5px"}}
     ...
     #_(when (some? extract-query-pages-ref?)
         [:> Divider]
         [:div.chk
          {:style {:align-self "center"
                   :margin-left "5px"}}
